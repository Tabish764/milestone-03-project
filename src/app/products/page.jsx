...
const Page = async () => {
  ...
  try {
    ...
    const res = await fetch(
      "https://milestone-03-project-sepia.vercel.app/api/products",
      {
        cache: "no-cache",
      }
    );
  ...
  return (
    <div className="flex max-w-7xl mx-auto  gap-[50px] pt-[50px] text-center flex-wrap items-center justify-center px-5 ">
      ...
      {data &&
        data.map((item) => (
          <div
            className="bg-[#222222]   delay-100 transition-all flex gap-2 card flex-col items-center w-[20rem] px-5 h-[36rem] p-4 rounded-md mb-5 mt-5"
            key={item.id}
          >
            ...
            <div className="flex gap-2">
              <Cart data={item} />
              <Link
                className="bg-red-600 py-2 px-4 rounded-lg mt-auto"
